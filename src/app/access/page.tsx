...
export default function AccessPage() {
  ...
  const fetchPages = async () => {
    ...
    try {
      ...
      // If default pages don't exist in the database, create them
      if (pagesData.length === 0) {
        await initializeDefaultPages();
      } else {
        // Check if all default pages exist
        const defaultSlugs = ['about', 'events', 'contact'];
        const existingSlugs = pagesData.map((page: any) => page.slug);
        const missingSlugs = defaultSlugs.filter(slug => !existingSlugs.includes(slug));
        
        if (missingSlugs.length > 0) {
          await initializeDefaultPages(missingSlugs);
          // Refresh pages after initialization
          const updatedPagesRes = await fetch('/api/pages');
          if (updatedPagesRes.ok) {
  ...
  // Initialize default pages in the database
  const initializeDefaultPages = async (slugsToCreate?: string[]) => {
    try {
      // Get default content from the ContentPage component
      ...
      // Create specified pages or all default pages if not specified
      const pagesToCreate = slugsToCreate || Object.keys(defaultContent);
      
      for (const slug of pagesToCreate) {
        const pageData = defaultContent[slug as keyof typeof defaultContent];
        if (pageData) {
      ...
      if (statusEl) {
        statusEl.textContent = 'Default pages initialized';
        setTimeout(() => {
  ...
  // Fetch all admin data (images and pages)
    ...
    <>
      ...
      >
        ...
        {activeTab === 'images' ? (
            ...
            >
              ...
              {loadingImages ? (
                ...
                <div 
                  ...
                  aria-live="polite"
                >
                  Žádné obrázky nenalezeny. Nahrajte první výše!
                </div>
              ): (
                <div 
                  style={{ 
          ...
          // Pages Management UI
          <>
            {editingPage || creatingPage ? (
              <PageEditor 
                pageSlug={creatingPage ? undefined : editingPage} 
                onClose={handlePageEditorClose} 
              />
            ) : (
              <div>
                <div style={{ 
                ...
                {loadingPages ? (
                  ...
                  <div style={{ 
                    ...
                    <p style={{ color: "#6B7280", marginBottom: "16px" }}>Žádné stránky nenalezeny.</p>
                    <button
                      onClick={() => setCreatingPage(true)}
                      style={{
                        padding: '8px 16px',
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                          ...
                          <th style={{ textAlign: 'left', padding: '12px 16px', fontWeight: '600' }}>URL</th>
                      ...
                      <tbody>
                        {pages.map((page) => (
                          <tr key={page.slug} style={{ borderBottom: '1px solid #e5e7eb' }}>
                            <td style={{ padding: '12px 16px' }}>{page.title}</td>
                            <td style={{ padding: '12px 16px' }}>/{page.slug}</td>
